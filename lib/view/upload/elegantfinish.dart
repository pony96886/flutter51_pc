...
class ElegantFinishState extends State<ElegantFinish> {
  ...
  Future onSendData(Map fileData) async {
    //原图片列表（包括编辑过）
    List _image =
        (UploadFileList.allFile['image']?.originalUrls ?? []).map((e) {
    ...
    List _newImage = (fileData['image'] ?? []).map((e) {
      return {
        'url': e['url'],
        'cover': e['w'].toString() + "," + e['h'].toString()
      };
    }).toList();
    //原视频列表（包括编辑过）
    List _video =
        (UploadFileList.allFile['video']?.originalUrls ?? []).map((e) {
        ...
        body: SingleChildScrollView(
          child: Column(
              ...
              children: <Widget>[
                Padding(
                  padding:
                      EdgeInsets.symmetric(vertical: 15.w, horizontal: 15.5.w),
                  child: Column(
                    ...
                    children: <Widget>[
                      ...
                      UploadResouceWidget(
                        ...
                        initResouceList: (widget.video ?? []).map((e) {
                          return FileInfo(
                              e['url'], 0, 1, 'video', 'video', null, 0, 0);
                        }).toList(),
                      ),
                      (widget.elegantdata!['video_valid'] == '1' ||
                                  widget.elegantdata!['vvip'] == '1') &&
                              widget.images == null
                          ? Padding(
                      ...
                      GestureDetector(
                        onTap: () {
                          if (((UploadFileList.allFile['image']?.originalUrls ??
                                          [])
                                      .length +
                                  (UploadFileList.allFile['image']?.urls ?? [])
                                      .length) ==
                              0) {
                            CommonUtils.showText('请上传最新茶女郎照片');
                            return;
                          }
                          if ((widget.elegantdata!['video_valid'] == '1' ||
                                  widget.elegantdata!['vvip'] == '1') &&
                              (UploadFileList.allFile['authVideo']?.urls ?? [])
                                      .length ==
                                  0 &&
                              widget.images == null) {
