...
class CityTree {
  ...
  Map<String, dynamic>? hotcitiesInfo;
  ...
  CityTree({this.metaInfo, this.provincesInfo, this.hotcitiesInfo});

  Map<String, dynamic>? get _metaInfo => this.metaInfo ?? citiesData;
  Map<String, dynamic>? get _provincesData =>
      this.provincesInfo ?? provincesData;
  Map<String, dynamic>? get _hotcitiesInfo =>
      this.hotcitiesInfo ?? hotcitiesInfo;
...
//    }
    ...
    tree = _buildTree(root, _metaInfo![provinceId.toString()], _metaInfo!);
    _cache.set(_cacheKey, tree);
    return tree;
  }

  initTreeHot(int hotcitieId) {
    String _cacheKey = hotcitieId.toString();
    String? name = this._hotcitiesInfo![hotcitieId.toString()];
    String letter = PinyinHelper.getFirstWordPinyin(name!).substring(0, 1);
    var root =
        new Point(code: hotcitieId, letter: letter, child: [], name: name);
    tree = _buildTree(root, _metaInfo![hotcitieId.toString()], _metaInfo!);
    _cache.set(_cacheKey, tree);
    return tree;
  }
  ...
  int? _getProvinceByCode(int code) {
    ...
    List<String> keys = _metaInfo!.keys.toList();
    for (int i = 0; i < keys.length; i++) {
      String key = keys[i];
      Map<String, dynamic> child = _metaInfo![key];
...
class Provinces {
  ...
  Map<String, dynamic> get _metaInfo => this.metaInfo ?? provincesData;
