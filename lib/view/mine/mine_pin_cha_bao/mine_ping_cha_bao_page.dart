...
class MinePingChaBaoPage extends StatefulWidget {
  @override
  _MinePingChaBaoPageState createState() => _MinePingChaBaoPageState();
}

class _MinePingChaBaoPageState extends State<MinePingChaBaoPage> {
  // ignore: unused_field
  bool _tabState = true;
...
class _ZhiChuState extends State<ZhiChu> {
  ...
  getList() async {
    ...
    Map? entity = await pcbList(page: _page, type: widget.type == 1 ? spenssa : incomeStr);
...
class _PCBItemState extends State<PCBItem> {
  ...
  Widget build(BuildContext context) {
    ...
    String titleAvatar = data['resources'].length > 0 ? data['resources'][0]['img_url'] : '';
    ...
    String status = statusCode == 1
        ...
        : statusCode == 2
            ...
            : statusCode == 3
                ? tgqxStr
                : "";
    Color statusColor = statusCode == 1 ? StyleTheme.cDangerColor : StyleTheme.cTextColor;
...
//                      color: Colors.amberAccent,
                  ...
                  Expanded(
                    child: Text(
                      nickname,
                      style: TextStyle(fontSize: 16.w, color: StyleTheme.cTitleColor, fontWeight: FontWeight.bold),
                    ),
                  ),
                  Text(
                    status,
                        ...
                        Column(
                          ...
                          children: <Widget>[
                            Text(
                              title,
                              style:
                                  TextStyle(fontSize: 18.w, color: StyleTheme.cTitleColor, fontWeight: FontWeight.bold),
                            ),
                            Text(
                              titleContent,
                              style: TextStyle(
              ...
              Row(
                ...
                children: <Widget>[
                  ...
                  _statusCode != 1
                      ...
                      : GestureDetector(
                          onTap: () {
                            CgDialog.cgShowDialog(context, widget.type == 2 ? qxfkStr : qrfkStr,
                                widget.type == 2 ? qxfktipsStr : qrfkTipsStr, ['取消', '确认'], callBack: () async {
                              String msg = '发生未知错误';
                ...
                Column(
                  ...
                  children: <Widget>[
                    Center(
                      child: Text(
                        '取消托管',
                        style: TextStyle(color: StyleTheme.cTitleColor, fontSize: 18.w, fontWeight: FontWeight.bold),
                      ),
                    ),
                    Flexible(
                              ...
                              children: <Widget>[
                                Container(
                                  ...
                                  child: Text(
                                    '取消原因',
                                    style: TextStyle(
                                        color: StyleTheme.cTitleColor, fontSize: 16.sp, fontWeight: FontWeight.w500),
                                  ),
                                ),
                          ...
                          GestureDetector(
                            onTap: () async {
                              if (appealController.text == '') {
                                BotToast.showText(text: '请填写申诉原因', align: Alignment(0, 0));
                                return;
                              } else {
                                Navigator.of(context).pop({'text': appealController.text});
                              }
                            },
                            child: Container(
                              margin: new EdgeInsets.only(top: 30.w),
