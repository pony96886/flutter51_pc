...
class HiveBoxCache<K, V> implements ICache<K, V> {
  HiveBoxCache(this.box);

  final LazyBox<V> box;
  @override
  Future<void> upsert(K key, V value) async => await box.put(key, value);

  @override
  Future<V?> read(K key) async => await box.get(key);

  @override
  Future<void> delete(K key) async => await box.delete(key);
...
class _ImageCacheManager implements ImageCacheManager {
  bool _isInitialized = false;

  @override
  late final ICache<String, Uint8List> cache;
  ...
  @override
  late final String? boxPath;

  @override
  Future<void> clearCache() => cache.clearAll();

  @override
  Future<void> init(String boxKey, {String? salt}) async {
    ...
    _salt = salt ?? '';
    final box = await Hive.openLazyBox<Uint8List>(boxKey);
    cache = HiveBoxCache<String, Uint8List>(box);
    boxPath = box.path;
  }
  ...
  Future<void> clearImageCacheIfNeed({bool force = false}) async {
    if (force || kIsWeb || _imageCacheManager.boxPath == null) {
      await _imageCacheManager.clearCache();
      return;
    }

    final file = File(_imageCacheManager.boxPath!);
...
class _ImageCache extends ImageCache {
        ...
        if (key.headers != null) {
          ...
          if (isContains) {
            result = MultiFrameImageStreamCompleter(
              codec: () async {
                final imageData = await _imageCacheManager.cache.read(cacheKey);
                if (imageData == null) {
                  await _imageCacheManager.cache.delete(cacheKey);
                  scheduleMicrotask(() {
  ...
  Future<ui.Codec> _instantiateImageCodec(
    Uint8List decrypted,
  ) async {
...
class _ImageCacheHooker extends WidgetsFlutterBinding {
  @override
