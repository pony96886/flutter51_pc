...
class _TCardState extends State<TCard> with TickerProviderStateMixin {
  ...
  void _forwardCallback() {
    _frontCardIndex++;
    _return();
    if (widget.onForward is Function) {
      widget.onForward!(
        _frontCardIndex,
    ...
    if (widget.onEnd is Function && _frontCardIndex >= _cards.length) {
      widget.onEnd!();
    }
  }
  ...
  void _judgeRunAnimation(DragEndDetails details, Size size) {
    ...
    if ((isSwipLeft || isSwipRight) && _frontCardIndex != _cards.length - 1) {
      ...
      if (_frontCardIndex == _cards.length - 1) {
        BotToast.showText(text: '已经是最后一位花魁了哦～', align: Alignment(0, 0));
      }
  ...
  void initState() {
    ...
    // 绑定控制器
    if (widget.controller is TCardController) {
  ...
  Widget build(BuildContext context) {
    if (_cards.length != widget.cards.length) {
      setState(() {
        _cards = widget.cards;
      });
    }
