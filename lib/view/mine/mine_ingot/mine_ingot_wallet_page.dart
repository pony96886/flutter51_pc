...
class MineIngotWalletPage extends StatefulWidget {
  MineIngotWalletPage({Key? key}) : super(key: key);

  @override
  State<StatefulWidget> createState() => MineIngotWalletPageState();
}

class MineIngotWalletPageState extends State<MineIngotWalletPage> with PayMixin {
  bool loading = true;
          ...
          if (val!['status'] != 0)
            {
              ...
              Provider.of<GlobalState>(context, listen: false).setFreezeMoney(val['data']['freeze_money'])
            }
        });
    if (!kIsWeb) {
  ...
  Widget build(BuildContext context) {
    ...
    String moneyStr = CommonUtils.renderFixedNumber(double.parse(money.toString()));
    String freezeMoneyStr = CommonUtils.renderFixedNumber(double.parse(freezeMoney.toString()));
        ...
        appBar: PreferredSize(
            child: PageTitleBar(
              ...
              rightWidget: GestureDetector(
                  onTap: () {
                    AppGlobal.appRouter?.push(CommonUtils.getRealHash('rechargeRecord/2'));
                  },
                  child: Center(
                    child: Container(
                ...
                : Padding(
                    ...
                    child: Column(
                      children: <Widget>[
                            ...
                            child: Stack(
                              children: [
                                LocalPNG(
                                  width: double.infinity,
                                  height: 180.w,
                                  url: 'assets/images/nigotwallet/money-bg-2.png',
                                  fit: BoxFit.cover,
                                ),
                                Padding(
                                          ...
                                          children: <Widget>[
                                            GestureDetector(
                                              ...
                                              child: LocalPNG(
                                                height: 50.w,
                                                width: 134.5.w,
                                                url: "assets/images/mymony/withdraw.png",
                                                fit: BoxFit.cover,
                                              ),
                                            ),
                                            GestureDetector(
                                              ...
                                              child: LocalPNG(
                                                height: 50.w,
                                                width: 134.5.w,
                                                url: "assets/images/mymony/recording.png",
                                                fit: BoxFit.cover,
                                              ),
                                            )
                                          ],
                        ...
                        GestureDetector(
                          onTap: () {
                            AppGlobal.appRouter?.push(CommonUtils.getRealHash('exchangemember'));
                          },
                          child: Container(
                              ...
                              child: Padding(
                                ...
                                child: Row(
                                  ...
                                  children: [
                                    ...
                                    Icon(Icons.keyboard_arrow_right, color: StyleTheme.cTextColor),
                                  ],
                                ),
                              )),
                        ),
      ...
      child: Container(
        ...
        child: Column(
          ...
          children: <Widget>[
            ...
            Text(
              '¥$promoPrice',
              style: TextStyle(color: Color(0xFF505050), fontSize: 24.sp, fontWeight: FontWeight.bold),
            )
          ],
        ),
      ),
...
class _RechargeModelState extends State<RechargeModel> {
  ...
  Future onSubmit() async {
    ...
    var result =
        await onCreatePay(channelItem['channel'], channelInfo['name'], widget.product!['id'], code: UserInfo.imageCode);
    ...
    if (result['status'] != 1) {
      ...
      var res = await onCreatePay(channelItem['channel'], channelInfo['name'], widget.product!['id'],
  ...
  Widget build(BuildContext context) {
    return Container(
      height: MediaQuery.of(context).size.height * 0.54 + ScreenUtil().bottomBarHeight,
...
class ChannelContainer extends StatefulWidget {
  ...
  ChannelContainer({Key? key, this.channel, this.selectChannel, this.onTabs}) : super(key: key);

  @override
  _ChannelContainerState createState() => _ChannelContainerState();
}

class _ChannelContainerState extends State<ChannelContainer> with SingleTickerProviderStateMixin {
  TabController? controller;
  int _index = 0;
  @override
  void initState() {
    super.initState();
    controller = new TabController(vsync: this, length: widget.channel!.length, initialIndex: 0);
      ...
      appBar: AppBar(
        ...
        title: TabBar(
            ...
            tabs: widget.channel!
                .asMap()
                .keys
                .map((key) => CustomTab(tabIndex: _index, keyIndex: key, channelItem: widget.channel![key]))
                .toList()),
      ),
      backgroundColor: Colors.white,
      body: TabBarView(
...
class CustomTab extends StatelessWidget {
  ...
  const CustomTab({Key? key, this.tabIndex, this.keyIndex, this.channelItem}) : super(key: key);
...
class _ListViewTileState extends State<ListViewTile> with AutomaticKeepAliveClientMixin {
  int selectIndex = 0;
              ...
              child: Row(
                children: <Widget>[
                  SizedBox(
                    ...
                    height: 40.w,
                    child: ImageNetTool(
                      url: widget.payway![index]['img_url'].indexOf('http') == -1
                          ? AppGlobal.bannerImgBase + widget.payway![index]['img_url']
