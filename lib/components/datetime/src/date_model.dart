...
class DateTimePickerModel extends CommonPickerModel {
  ...
  DateTimePickerModel(
    ...
    if (currentTime != null) {
      ...
      if (this.minTime!.isAfter(now)) {
        this.currentTime = this.minTime;
      } else if (this.maxTime!.isBefore(now)) {
    ...
    if (isAtSameDay(this.minTime!, this.currentTime!)) {
  ...
  void setLeftIndex(int? index) {
    ...
    if (isAtSameDay(minTime!, time)) {
      int index = min(24 - minTime!.hour - 1, _currentMiddleIndex!);
      this.setMiddleIndex(index);
    } else if (isAtSameDay(maxTime!, time)) {
      int index = min(maxTime!.hour, _currentMiddleIndex!);
      this.setMiddleIndex(index);
    }
  }
  ...
  String? leftStringAtIndex(int? index) {
    DateTime time = currentTime!.add(Duration(days: index!));
    if (time.isBefore(minTime!) && !isAtSameDay(minTime!, time)) {
      return null;
    } else if (time.isAfter(maxTime!) && !isAtSameDay(maxTime!, time)) {
      return null;
    }
  ...
  DateTime finalTime() {
    ...
    if (currentTime == null ||
        minTime == null ||
        _currentLeftIndex == null ||
        _currentMiddleIndex == null ||
        _currentRightIndex == null) {
      throw Exception("必要字段不能为空");
    }
