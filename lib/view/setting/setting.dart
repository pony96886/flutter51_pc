...
class SettingPageState extends State<SettingPage> {
    ...
    BotToast.showCustomText(
        toastBuilder: (_) => Container(
              ...
              decoration: BoxDecoration(
                  ...
                  border: Border.all(
                      color: Colors.white.withOpacity(0.12), width: 0.5.w),
  ...
  Future<void> checkVersion() async {
    try {
      ...
      var _versionLocal =
          Platform.isIOS ? '${info.version}.${info.buildNumber}' : info.version;
  ...
  void showUpdate() async {
    ...
    UpdateModel.showUpdateDialog(backButtonBehavior, cancel: () {
      ...
      Provider.of<SignInConfig>(context, listen: false).setShow(false);
        ...
        version: "茶馆儿v.${versionMsg.version}",
        mustupdate: versionMsg.must == 1,
                ...
                child: Container(
                  ...
                  child: Column(children: <Widget>[
                    Text(
                      '选择一个你喜欢的头像',
                      style:
                          TextStyle(color: Color(0xFF323232), fontSize: 15.sp),
                    ),
                    SizedBox(
                      height: 15.w,
                    ),
                      ...
                      child: SingleChildScrollView(
                        child: Wrap(
                          ...
                          children: _avatars
                              .asMap()
                              .entries
                              .map((MapEntry map) => _avatarBox(state, map.key))
                              .toList(),
                        ),
                      ),
                    ),
  ...
  Widget build(BuildContext context) {
    ...
    if (memberInfo != null) {
      ...
      if (['', null, false].contains(memberInfo.phone)) {
        ...
        String d4Sds = r'\d{4}';
        _phonenumbers = mobilephone.length > 4
            ? mobilephone.replaceFirst(new RegExp(d4Sds), '****', 3)
            : null;
      }
          ...
          child: SingleChildScrollView(
            child: Column(
                ...
                children: <Widget>[
                  ...
                  ListTile(
                    onTap: () {
                      InputDialog.show(context, '昵称', limitingText: 14)
                          .then((value) {
                        if (value != '' && value != null) {
                                ...
                                onTap: () {
                                  if (_phonenumbers == null) {
                                    AppGlobal.appRouter?.push(
                                        CommonUtils.getRealHash('loginPage/2'));
                                  }
                                },
                                title: Text(
                                    _phonenumbers == null ? bindphoneT : '账号',
                                    style: TextStyle(
                                ...
                                trailing: Row(
                                  ...
                                  children: <Widget>[
                                    Text(
                                        _phonenumbers == null
                                            ? '去注册'
                                            : _phonenumbers.toString(),
                                        style: TextStyle(
                                            fontSize: 14.sp,
                                            color: _phonenumbers == null
                                                ? StyleTheme.cBioColor
                                                : StyleTheme.cTitleColor)),
                                    Icon(Icons.keyboard_arrow_right,
                                        color: StyleTheme.cTitleColor),
                        ...
                        return SwitchListTile(
                          ...
                          onChanged: (bool value) {
                            PersistentState.getState('initApp').then((initapp) {
                              ...
                              if (initapp == null) {
                                context
                                    .push(CommonUtils.getRealHash('tealist'));
                              } else {
                  ...
                  ListTile(
                    onTap: () {
                      InputDialog.show(context, '兑换码', limitingText: 40)
                          .then((value) {
                        if (value != '' && value != null) {
                    ...
                    trailing:
                      ...
                      Text(
                          _redemptioncode == null
                              ? "输入兑换码"
                              : _redemptioncode.toString(),
                          style: TextStyle(
                              fontSize: 14.sp,
                              color: _redemptioncode == null
                                  ? StyleTheme.cBioColor
                                  : Color(0xFF323232))),
                      Icon(Icons.keyboard_arrow_right,
                          color: StyleTheme.cTitleColor)
                  ...
                  ListTile(
                    onTap: () {
                      InputDialog.show(context, '邀请码', limitingText: 6)
                          .then((value) {
                        if (value != '' && value != null) {
                    ...
                    trailing:
                      ...
                      Text(
                          _invitationcode == null
                              ? "输入邀请码"
                              : _invitationcode.toString(),
                          style: TextStyle(
                              fontSize: 14.sp,
                              color: _invitationcode == null
                                  ? StyleTheme.cBioColor
                                  : Color(0xFF323232))),
                      Icon(Icons.keyboard_arrow_right,
                          color: StyleTheme.cTitleColor)
                  ...
                  ListTile(
                    onTap: () {
                      if (memberInfo.email!.isEmpty) {
                        AppGlobal.appRouter
                            ?.push(CommonUtils.getRealHash('bindPhone'));
                      } else {
                        BotToast.showText(
                            text: '已绑定邮箱', align: Alignment(0, 0));
                      }
                    },
                    title: Text('绑定邮箱',
                        style: TextStyle(
                  ...
                  ListTile(
                    ...
                    title: Text('版本号',
                        style: TextStyle(
                            color: StyleTheme.cTitleColor, fontSize: 16.sp)),
                    trailing: Text(getVersion(),
                        style: TextStyle(
                            fontSize: 14.sp, color: Color(0xFF323232))),
                  ),
                  LoginOutButton(
                    onTap: () {
                      _handleSafeLoginOut();
...
class LoginOutButton extends StatelessWidget {
          ...
          actions: <Widget>[
            ElevatedButton(
              ...
              child: Text(
                "取消",
                style:
                    TextStyle(fontSize: 14.sp, color: StyleTheme.cDangerColor),
              ),
            ),
            ElevatedButton(
              style: ElevatedButton.styleFrom(
  ...
  Widget build(BuildContext context) {
    bool isLogin = false;
    if (['', null, false]
        .contains(Provider.of<HomeConfig>(context).member.phone)) {
      isLogin = false;
    } else {
      isLogin = true;
    }
