...
class ElegantCity extends StatefulWidget {
  // cityList 传入格式 [{code:110000,city:"北京"},{code:110000,city:"北京"}]
  final List? cityList;
  final bool? showAll;

  const ElegantCity({Key? key, this.cityList, this.showAll = false})
      : super(key: key);
...
class PageState extends State<ElegantCity> with SingleTickerProviderStateMixin {
  List<String> letters = [];
  List<Datum>? data;
  Timer? _changeTimer;
  ...
  Future<List<dynamic>> onOrderData({List? tmpCityList}) async {
    // List cityList = widget.cityList;
    List cityList = tmpCityList!;
    var allCityCodeList = [];
    ...
    for (var i = 0; i < cityList.length; i++) {
      allCityCodeList.add({
        ...
        "letter":
            PinyinHelper.getFirstWordPinyin(cityList[i]['city']).substring(0, 1)
      });
    }
  ...
  getLettersDataList() async {
    var result = await onOrderData(tmpCityList: widget.cityList);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      //   title: Text("雅间城市选择",
      //       style: TextStyle(
      //           color: cTitleColor, fontSize: GVScreenUtil.setSp(36))),
      //   centerTitle: true,
...
class ChinaCity extends StatefulWidget {
  ...
  const ChinaCity({Key? key, this.cityList, this.showAll}) : super(key: key);
...
class AbroadCity extends StatefulWidget {
  ...
  const AbroadCity(
      {Key? key, this.cityList, this.showAll = false, this.isDaTing = false})
      : super(key: key);
...
class _AbroadCityState extends State<AbroadCity> {
  ...
  Future<List<dynamic>> onOrderData({List? tmpCityList}) async {
    ...
    // 获取城市首字母
    for (var i = 0; i < cityList.length; i++) {
      String city = cityList[i]['name'];
      // print('---------------------------adsfasfaf     $city');

      if (city.isNotEmpty) {
        String letter =
            PinyinHelper.getFirstWordPinyin("$city").substring(0, 1);
        allCityCodeList.add(
            {"code": cityList[i]['cityCode'], "city": city, "letter": letter});
      }
    }
    var baseAllCityCode = allCityCodeList.toList();
    var allCityList = [];
  ...
  // getLettersDataList() async {
  //   Map tmpdata = await getAbroadCity();
  //   if (tmpdata == null) {
  //     tmpdata = {"data": []};
  //   }
  //   var result = await onOrderData(tmpCityList: tmpdata['data']);
  //   var resultString = {"data": result};
  //   var resultEntity = new CityListData.fromJson(resultString);
