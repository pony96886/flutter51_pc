  ...
  void initState() {
    ...
    getRequireDetail(widget.id!).then((res) {
      if (res!['status'] != 0) {
        ...
        loading = false;
        String serTypeStr =
            detailData!['serviceType'].replaceAll(',', '、').toString();
        String costWayStr =
            detailData!['costWay'].replaceAll(',', '、').toString();
        itemList = [
          {
            'icon': 'assets/images/detail/icon-postion.png',
            'title': '城市: ',
            'detail': detailData!['cityName'] ?? '--'
          },
          {
            'icon': 'assets/images/detail/icon-time.png',
            'title': '时间: ',
            'detail': '最晚' + detailData!['latestTime']
          },
          {
            ...
            'detail': costWayStr +
                ...
                serTypeStr +
                "、最高接受" +
                detailData!['highestPrice'].toString()
          },
          {
                        ...
                        child: Column(
                          ...
                          children: <Widget>[
                            for (var item in itemList!)
                              hallItem(
                                  item['icon'], item['title'], item['detail']),
                            detailData!['status'] == 1
                                ? Container(
      ...
      child: Row(
        ...
        children: <Widget>[
          ...
          Text(
            title,
            style: TextStyle(
                height: 1.2, color: StyleTheme.cTitleColor, fontSize: 14.sp),
          ),
          (detail is String)
              ? Flexible(
                  child: Text(
              ...
              : Expanded(
                  ...
                  child: detail.length > 0
                      ? Wrap(
                          ...
                          runSpacing: 10.w,
                          children: <Widget>[
                            for (var item in detail) yjTag(item)
                          ],
                        )
                      : Text('--'),
                ))
    ...
    return Container(
      ...
      child: Stack(
        children: [
          LocalPNG(
              height: 15.w,
              url: 'assets/images/card/tag-bg-$value.png',
              fit: BoxFit.fill),
          Padding(
            ...
            child: Column(
              ...
              children: <Widget>[
                Text(
                  title,
                  style:
                      TextStyle(fontSize: 10.w, color: StyleTheme.cDangerColor),
                ),
              ],
            ),
          ),
