...
class MinePromotionRecordPage extends StatefulWidget {
  @override
  State<StatefulWidget> createState() => MinePromotionRecordPageState();
}

class MinePromotionRecordPageState extends State<MinePromotionRecordPage> {
  int _allNumber = 0;
  bool loadmore = true;
  int _page = 1;
  int _limit = 10;
  ...
  Future getInvition(_page, _limit) async {
    ...
    if (result!['status'] == 1) {
      if (_page == 1) {
        ...
        setState(() {
          isAll = (result['data']['list'] == null || result['data']['list'].length < _limit);
              ...
              child: Column(
                children: <Widget>[
                  Container(
                    ...
                    padding: new EdgeInsets.only(top: 10.5.sp, bottom: 10.5.sp, left: 15.5.sp),
...
class RecordItem extends StatelessWidget {
  ...
  const RecordItem({Key? key, this.nickname, this.created, this.register}) : super(key: key);

  @override
  Widget build(BuildContext context) {
    var times = new DateTime.fromMillisecondsSinceEpoch(int.parse(created!) * 1000);
    String _createdtime = '${times.year}-${times.month}-${times.day} ${times.hour}:${times.minute}';
        ...
        children: [
          ...
          Text(
            ...
            style: TextStyle(
              ...
              color: register == "未注册" ? StyleTheme.cDangerColor : StyleTheme.cBioColor,
            ),
          ),
        ],
      ),
