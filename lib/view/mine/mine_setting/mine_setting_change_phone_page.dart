...
class MineSettingChangePhone extends StatefulWidget {
  final String? phone;
  final String? phonePrefix;
  final String? showPhone;

  const MineSettingChangePhone({Key? key, this.phone, this.phonePrefix, this.showPhone}) : super(key: key);
  @override
  State<StatefulWidget> createState() => MineSettingChangePhoneState();
}

class MineSettingChangePhoneState extends State<MineSettingChangePhone> {
  ...
  Future setCodeData(int? type, Function? callBack) async {
    if (type == 1) {
      var data = await getCaptcha(_phoneNumController.text, phonePrefix.replaceAll("+", ""), 4, UserInfo.imageCode!);
      if (data!['status'] == 1) {
        setState(() {
          codeSending = true;
        });
      ...
      var data =
          await getCaptcha(_newphoneNumController.text, newphonePrefix.replaceAll("+", ""), 4, UserInfo.imageCode!);
      if (data!['status'] == 1) {
        setState(() {
          newcodeSending = true;
        });
                ...
                child: SizedBox(
                  ...
                  child: Stack(
                    children: [
                      ...
                      Center(child: Text("确认更换", style: TextStyle(color: Colors.white, fontSize: 15.sp))),
                    ],
                  ),
                ),
              ),
              SizedBox(height: 20.w),
              Center(child: Text("更换后原手机号将不能用于登录", style: TextStyle(color: StyleTheme.cDangerColor, fontSize: 12.sp))),
            ],
          ),
        ),
      ),
