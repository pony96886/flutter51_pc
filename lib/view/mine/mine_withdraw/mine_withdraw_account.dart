...
class MineWithdrawAccountPage extends StatefulWidget {
  final String? selectId;
  MineWithdrawAccountPage({Key? key, this.selectId}) : super(key: key);

  @override
  _MineWithdrawAccountPageState createState() => _MineWithdrawAccountPageState();
}

class _MineWithdrawAccountPageState extends State<MineWithdrawAccountPage> {
  int? selectID;
  bool loading = true;
  Map? inputStatr;
  List? _accountList;
      ...
      child: Scaffold(
              ...
              rightWidget: GestureDetector(
                  ...
                  onTap: () async {
                    if (_accountList!.length == 5) {
                      BotToast.showText(text: '最多只能添加五个账户哦～', align: Alignment(0, 0));
                      return;
                    }
                    await showAddDialog();
                    addUserAccount(inputStatr!['cardCode'], inputStatr!['cardName']);
                  },
                  child: Center(
                    child: Container(
        ...
        body: loading
            ...
            : (_accountList!.length == 0
                ...
                : Container(
                    padding: EdgeInsets.only(top: 10.w, bottom: 10.w + ScreenUtil().statusBarHeight),
                    child: ListView.separated(
                        ...
                        itemBuilder: (BuildContext context, int index) {
                          return _accountRadio(_accountList![index]['id'], _accountList![index]['name'],
    ...
    return InkWell(
        onTap: () {
          ...
          EventBus().emit('changeAcount',
              {'accountName': name, 'accountId': account, 'selectId': id.toString(), 'selectUser': true});
        },
        child: Container(
                ...
                onTap: () async {
                  showDeleteDialog().then((value) {
                    if (value!) {
                      ...
                      if (id == selectID) {
                        EventBus().emit('changeAcount',
                            {'accountName': null, 'accountId': '', 'selectId': null, 'selectUser': false});
                      }
                      deleteAccout(id);
                    }
                  });
                    ...
                    children: <Widget>[
                      ...
                      Container(
                          ...
                          child: Text(
                            '确定删除该账户吗？',
                            style:
                                TextStyle(fontSize: 16.sp, fontWeight: FontWeight.bold, color: StyleTheme.cTitleColor),
                          )),
                      GestureDetector(
                        onTap: () {
                          context.pop();
              ...
              children: <Widget>[
                Column(
                  ...
                  children: <Widget>[
                    ...
                    Container(
                      margin: new EdgeInsets.only(top: 30.w),
                      padding: new EdgeInsets.symmetric(horizontal: 10.w, vertical: 8.w),
                      decoration: BoxDecoration(
                          color: StyleTheme.bottomappbarColor, borderRadius: BorderRadius.circular(22.5.w)),
                      child: TextField(
                    ...
                    GestureDetector(
                      onTap: () {
                        if (cardName.text == '') {
                          BotToast.showText(text: '请输入正确的持卡人姓名～', align: Alignment(0, 0));
                        } else {
                          if (cardCode.text.length > 10) {
                            inputStatr = {'cardCode': cardCode.text, 'cardName': cardName.text};
                            context.pop();
                            cardCode.text = '';
                            cardName.text = '';
                          } else {
                            BotToast.showText(text: '请输入正确的银行卡号～', align: Alignment(0, 0));
                          }
                          ;
                        }
                        ;
